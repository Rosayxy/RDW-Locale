...
async def user_loop(websocket, path):
    ...
    while True:
        ...
        print(data)
        if data["type"] == "start":
            physical_space = Space(data["physical"]["width"], data["physical"]["height"], data["physical"]["obstacle_list"])
            message = json.dumps({"type": "start"})
            await websocket.send(message)
        elif data["type"] == "running":
            user = UserInfo(data["physical"]["user_x"], data["physical"]["user_y"], data["physical"]["user_direction"], data["user_v"], data["user_w"]) # TODO: w
            delta_t = data["delta_t"]
            need_reset = data["need_reset"]
            if need_reset:
                user = update_reset(user, physical_space, delta_t)
                message = json.dumps({"type": "running", "user_x": user.x, "user_y": user.y, "user_direction": user.direction, "reset": True})
            else:
                has_reset=False
                if is_universal:
                    user, has_reset = update_user(user, physical_space, delta_t)
                    message = json.dumps({"type": "running", "user_x": user.x, "user_y": user.y, "user_direction": user.direction, "reset": has_reset})
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f','--file',default='client_logic.py')
    args = parser.parse_args()

    file_s=args.file
    is_universal=args.universal
    # Start the WebSocket server locally on the user's machine
